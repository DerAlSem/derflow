...
- **v4.2: архив утверждает истинность — а утверждать можно только увиденное.**
  Reap-гейт (v3.18) требовал архивировать заявку, но молчал о том, чем
  подтверждается её дельта. Следствие увидели 09.08.2026 в GM+: заявка
  `booking-group-payment-link` была заархивирована, дельта влита в живую спеку, и
  **три сценария из четырёх не были реализованы никогда**. Экран результата
  выпуска платёжной ссылки не видел ни один человек ни разу за всё время жизни
  фичи — блок сам себя перечитывал после выпуска и подменялся заглушкой загрузки,
  уничтожая только что показанный результат. Спека утверждала обратное, тесты
  были зелёные, ревью пройдено, все задачи отмечены.

  Вскрылось не проверкой, а жалобой менеджера «вы не сделали то, что я просил» —
  и он оказался прав против спеки, тестов и ревью одновременно.

  Ключевое, ради чего запись: **сценарий про увиденное чтением кода не
  закрывается.** Два дефекта из трёх ловились только взглядом на живой интерфейс —
  код был безупречен и просто не доезжал до экрана. Поэтому правило сформулировано
  не как «сделай sweep» (он поймал бы один из трёх), а как соответствие «род
  утверждения → чем подтверждается», с явным запретом закрывать «человек видит»
  чтением диффа.

  Гейт намеренно дешёвый: у честной дельты он не стоит ничего — ты и так смотрел.
