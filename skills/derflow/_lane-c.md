...
- **`/opsx:explore`** — мыслящий партнёр: читает код, взвешивает варианты,
  ...
  Раньше здесь стояло заимствование `superpowers:brainstorming` с
  переопределением его терминуса — пятнадцать строк на то, чтобы чужой скилл в
  конце брейншторма не увёл дизайн в `docs/superpowers/specs`. **С нативной
  командой этот отказ исчезает: чужому терминусу неоткуда взяться.**
...
- **`/opsx:update`** — пересмотр артефактов на полпути, **когда реализация вскрыла,
  ...
  «объём взорвался в другую работу». Раньше этого сценария в derflow не было вовсе,
  хотя он неизбежен.
...
разом и **сам детектит и разрешает конфликты спек**. Раньше здесь стояло ручное
правило «группируй по capability, не по `ls`» — накопленный долг перекрывает сам
себя, и слив в алфавитном порядке накатывает старую дельту поверх новой. Это
верно, но теперь это работа команды, а не твоей головы.

Что остаётся твоим: **порядок вытеснения**. Активные ID бездатные by design (см.
...
- **Меняешь существующую неспецированную сшивку? — просто пиши дельту на изменение (v4.0).** Правило апстрима, дословно: *«You do not document your whole codebase to start. You write specs only for what you're about to change.»* Спека густеет по трафику — область становится описанной после нескольких изменений в ней, а не по решению задокументировать всё заранее.
  ...
  Раньше здесь стоял реверс-вход через `adopt-code`: археология кода →
  `descriptive` SHALL'ы → промоушен в `normative` → sweep → baseline, и лишь
  потом обычный C. **Слой снят.** Он предполагал двухстатусную разметку, а по
  факту теги `normative`/`descriptive` нашлись в двух файлах спек из
  двадцати четырёх — то есть работа шла по упрощённой модели и без него.
  Археология стоит дорого, а платит только там, где baseline действительно нужен.
