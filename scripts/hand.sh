...
#
# Хендофф обязан нести строку «имя:» — она становится именем сессии и
# заголовком таба (`claude --name`). Конвенция — derflow/_capture.md.
#
...
# запись `?? .claude/`, `[ -f ]` на ней ложно, и `newest` выходит ПУСТЫМ — гейт
# молча инертен ровно там, где хендофф и лежит (замер 05.09.2026).
newest="$(cd "$dir" && git status --porcelain -uall 2>/dev/null | awk '{print $NF}' \
