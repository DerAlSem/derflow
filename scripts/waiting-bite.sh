...
#
# 🔴 ЗАМЕРЫ ПРИЁМКИ ПОСТАВКИ 2-2 «Пробуждение» (11.09.2026, 196 укусов).
# Все числа сняты на ОДНОМ И ТОМ ЖЕ стенде из 196 укусов — пересчитанных из
# прежних 181 нет ни одного: выведенное число в этой таблице было бы ложью с
# видом замера.
#
#   живой waiting.py .......................... ✅ 196 ❌ 0
#   заглушка (print + exit 0) ................. ✅ 0   ❌ 196
#
# Поломки первой приёмки:
#   probe_one: ненулевой код → silent ......... ✅ 193 ❌ 3
#   write_cache кладёт probe_rc в last_rc ..... ✅ 194 ❌ 2
#   unacked возвращает True всегда ............ ✅ 192 ❌ 4
#   wake печатает заголовок при пустой дельте . ✅ 193 ❌ 3
#   install_hook через json.load → json.dump .. ✅ 194 ❌ 2
#   probeable не отсеивает taken .............. ✅ 195 ❌ 1
#
# Поломки КРУГА КРИТИКИ (каждая — откат одной его починки):
#   cmd_wake ловит только Exception ........... ✅ 195 ❌ 1
#   stderr не уводится в никуда ............... ✅ 195 ❌ 1
#   строгий разбор UTF-8 в выводе пробы ....... ✅ 195 ❌ 1
#   reasons: состояние раньше формы ........... ✅ 194 ❌ 2
#   probeable без сторожа формы ............... ✅ 195 ❌ 1
#   hook_state грепает текст вместо разбора ... ✅ 195 ❌ 1
#   write_json с общим именем .tmp ............ ✅ 195 ❌ 1
#   registry_broken без watching .............. ✅ 195 ❌ 1
#
# Три замечания, купленные этими прогонами:
...
#   · 🔴 пер-строчный `try` в `cmd_probe` (одна строка не уносит весь прогон)
#     СОБСТВЕННОГО укуса не имеет, и это сказано вслух, а не спрятано: на
#     правильно оформленной строке ни один известный вход до него не доходит —
#     `re.error`, `OSError`, таймаут и разбор байтов перехвачены раньше и каждый
#     со своим укусом. Он стоит ради НЕИЗВЕСТНОГО класса, а укус на неизвестное
#     написать нельзя. Замерено: снятие только его стенд не красит.
#
