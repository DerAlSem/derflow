...
#
# 🔴 ЗАМЕРЫ ПРИЁМКИ ПОСТАВКИ 2-2 «Пробуждение» (11.09.2026, 181 укус).
#
#   живой waiting.py .......................... ✅ 181 ❌ 0
#   заглушка (print + exit 0) ................. ✅ 0   ❌ 181
#   probe_one: ненулевой код команды →
#     silent вместо unreachable ............... ✅ 178 ❌ 3
#   write_cache кладёт probe_rc в last_rc ..... ✅ 179 ❌ 2
#   unacked возвращает True всегда ............ ✅ 177 ❌ 4
#   wake печатает заголовок при пустой дельте . ✅ 179 ❌ 2
#   install_hook пишет json.load → json.dump .. ✅ 179 ❌ 2
#   probeable не отсеивает taken .............. ✅ 180 ❌ 1
#
# Три замечания, купленные этим прогоном:
#   · «unacked всегда True» ШИРЕ разметки плана (4 вместо ≥3): гашение по форме
#     ломается вместе с гашением по исходу — причина настоящая, укус не ослаб;
#   · «wake печатает заголовок» УЖЕ разметки (2 вместо ≥3): утверждений про ноль
#     байт в стенде ровно два, а не три. Промах счёта в плане, не в укусах — оба
#     названных краснеют;
#   · 🔴 json.dump сперва красил ТОЛЬКО укус про неоднозначный якорь, а «чужие
#     ключи целы» проходил. План назвал этот исход заранее и предписал завести
#     укус на БАЙТОВУЮ неизменность — заведён. Первая его редакция тоже была
#     слепа: фикстура была каноническим JSON с отступом 2, который
#     json.dumps(indent=2) воспроизводит байт в байт. Фикстуре дан массив В
#     СТРОКУ — файл, который правят люди и шесть сессий, каноническим не бывает.
#
