...
def display_path(p):
    ...
    образец entry — с путём: `~/dev/rk_bot · main`. Корень по умолчанию (Р7) —
    один `~/dev`, то есть под домашним каталогом лежит весь боевой случай; путь
    вне дома оставлен именем каталога — тем же, что писала задача 1.
    """
    p = pathlib.Path(p)
    try:
