...
⚠️ **Поправка 08.09.2026, найдена финальным ревью поставки — решение было верно
наполовину.** «Правка команд меняет `tree_sha` сама» верно ровно для ФИКСИРОВАННОГО
пути конфига. Флаг `--config`, который вводит эта же поставка, опору снимает: два
отслеживаемых конфига в одном дереве с разными `gate_pure` дают ОДИН ключ, и вердикт,
снятый по одному набору команд, читается как вердикт по другому. Воспроизведено: второй
гейт не гонялся вовсе, `memo` напечатал «гейт пройден» и вышел нулём. Покрыто СОДЕРЖИМОЕ
команд, не покрыта ИДЕНТИЧНОСТЬ прочитанного файла.

Починка ключ не трогает — Р2 остаётся в силе. Вердикт с самого начала записывал поле
`commands`, но `read_verdict` его никогда не читал; теперь читает и отвергает вердикт с
несовпадающими командами. Это лучше пути конфига в ключе: ловит и переименование, и
любую правку команд мимо `tree_sha`, а улика уже лежала в файле.
...
Ожидается `итог: ✅ 0   ❌ 45` — **ровно ноль зелёных**. Любой зелёный означает укус,
проходящий впустую: он сравнивает пустое с пустым, либо берёт иголкой строку, которую
инструмент печатает всегда. Найти, назвать и починить — это дефект укуса, а не заглушки.
Замер 08.09.2026: в задаче 1 так нашёлся ровно один такой укус из девяти.
...
Ожидается: 45 ✅, ❌ на всех укусах главы (`list` печатает «вердиктов нет» жёстко,
...
⚠️ **Число выросло с 50 при реализации 08.09.2026, и это не вольность.** Исполнитель
задачи 6 нашёл, что брифовый укус «`forget --all` вычистил оба вердикта» опирался на
«`list` ничего не печатает» — истинно и при сломанном `list`; заменил его проверкой
файловой системы со сверкой **до** (`было 2`), а не только **после** (`стало 0`), и
добавил два укуса на непокрытые ветки `forget` (голый вызов без аргументов; позиционный
`<sha>`). Плюс укус на сторож короткого префикса. Итог 53.
...
Ожидается `итог: ✅ 0   ❌ 53` — **ровно ноль зелёных**. Любой зелёный означает укус,
проходящий впустую: он сравнивает пустое с пустым, либо берёт иголкой строку, которую
инструмент печатает всегда. Найти, назвать и починить — это дефект укуса, а не заглушки.
Замер 08.09.2026: в задаче 1 так нашёлся ровно один такой укус из девяти.
...
- [ ] **Шаг 4б: сломать сторож ЗАПИСИ на грязном дереве**

```
cp ~/.claude/scripts/memo.py /tmp/memo-broken-5.py
```

В копии в `cmd_check` удалить ветку
...
то есть писать вердикт и тогда, когда кэшу доверять нельзя. Затем:

```
MEMO=/tmp/memo-broken-5.py bash ~/.claude/scripts/memo-bite.sh
```

Ожидается: **≥2 ❌** — «дерево снова чистое — берётся ПЕРВЫЙ вердикт, не переписанный»
и «при неотслеживаемом конфиге вердикт не записан ВОВСЕ».

⚠️ Эта поломка добавлена 08.09.2026 после того, как ревьюер задачи 5 применил её сам и
получил **полностью зелёный стенд**. Сторож в коде был, а укусы не отличали его наличие
от отсутствия: оба вердикта — законный и незаконный — целятся в один файл по одному
`tree_sha` и оба несут `pass`, поэтому «гейт пройден» печатается в обоих случаях.
Различает их только имя сессии. Урок шире одного укуса: **страховка от ЗАПИСИ не
проверяется чтением**, и утверждать её надо либо по имени писателя, либо прямо по
файловой системе.
